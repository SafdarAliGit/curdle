...
frappe.ui.form.on('Multan Branch', {
    refresh(frm) {
     ...
     $('input[name="bank_alfalah"]').prop('checked', false);
        ...
        if (frm.doc.docstatus == 1) {
            ...
            const print_format = 'POS Invoice Multan';
            ...
            printWindow.print();
            frappe.new_doc('Multan Branch');
        }
        var pos_profile = 'Multan Branch';
        ...
        $(document).ready(function () {
            ...
            if ($('.items-list').length === 0) {
                ...
                frappe.call({
                    method: 'curdle.curdle.doctype.utils.get_for_multan_branch', args: {
...
function initEventHandlers(frm) {
    $(document).off('click', '.items-list li').on('click', '.items-list li', function () {
        ...
        calucateTotal(frm);
        get_tax_rate(frm, frm.doc.mop, 'Multan Branch')
    });
...
// Event listener for number button clicks
    ...
    $(document).off('change', 'input[name="mode_of_payment"]').on('change', 'input[name="mode_of_payment"]', function () {
        ...
        get_tax_rate(frm, mop, 'Multan Branch');
